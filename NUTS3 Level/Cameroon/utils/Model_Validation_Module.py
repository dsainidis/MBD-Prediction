...
def plot_error_per_month(df, case=''):
    ...
    is_cat = pd.api.types.is_categorical_dtype(df['actual'])
    if is_cat:
        df['abs(error)'] = np.abs(df['actual'].cat.codes-df['prediction'])
    else:
...
def evaluate_xgboost(model, train, test = None, date_col = 'dt_placement', filepath='', case = '', fi = False, error_buffer=3):
    ...
    return results_train, results_test, test, date, mosq_col
